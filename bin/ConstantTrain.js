...
Main.main = function() {
	...
	var polyI = cstrain_core_Polynomial.fromCoefs([11,4,3,7]);
	polyI = polyI.add(cstrain_core_Polynomial.createDeg1x());
	console.log(cstrain_core_Polynomial.fromCoefs([1,0,0,0,1]).factorisation());
	console.log(cstrain_core_Polynomial.fromCoefs([-20,24,1,-6,1]).factorisation());
	console.log("End factorisation");
	var polyV = polyI.evalValueFloat(5);
	console.log(polyI.coefs);
	console.log(polyV);
	var poly = polyI.divisionWithRemainder(cstrain_core_Polynomial.createDeg1x());
	console.log(Std.string(poly[0].coefs) + " > " + Std.string(poly[1].coefs));
	console.log(poly[0].evalValueFloat(5) + " , " + (poly[0].evalValueFloat(5) + poly[1].evalValueFloat(5) / 5));
};
Math.__name__ = true;
...
cstrain_core_Polynomial.gcd_mult = function(ref) {
	...
	while(_g1 < _g) {
		var i = _g1++;
		if((ref[i] | 0) != 0) {
			d = cstrain_core_Polynomial.gcd(d,ref[i]);
		}
	}
	return d;
};
...
cstrain_core_Polynomial.prototype = {
	isRootFor: function(x) {
		if(this.coefs.length > 1) {
			return this.evalValueFloat(x) == 0;
		} else {
			return false;
		}
	}
	,findRoot: function() {
		...
		if(this.coefs[0] % 1 != 0 || this.coefs[this.coefs.length - 1] % 1 != 0) {
			console.log("Invalid not whole number coeffs!");
			return null;
		}
		var first = Math.abs(this.coefs[0]) | 0;
		var last = Math.abs(this.coefs[this.coefs.length - 1]) | 0;
		var _g1 = 1;
		var _g = last + 1;
		while(_g1 < _g) {
			var l = _g1++;
	...
	,factorisation: function() {
		...
		if(root != null) {
			factors.push(cstrain_core_Polynomial.fromCoefs([1,-root]));
	...
	,performOperation: function(op) {
		...
		case 3:
			...
			if(isVar3) {
				this.copyFrom(this.div(cstrain_core_Polynomial.createDeg1x()));
			} else {
				var _g11 = 0;
