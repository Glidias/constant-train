...
cstrain_core_Polynomial.getRepresentation = function(co,level,varLabel) {
	if(co != 0) return (co != 1 || level < 1?cstrain_core_Polynomial.floatToStringPrecision(co,2) + "":"") + (level >= 1?varLabel:"") + (level >= 2?"<sup>" + level + "</sup>":""); else return "";
};
...
cstrain_core_Polynomial.PrintOut = function(poly,varLabel) {
	var arr = [cstrain_core_Polynomial.getSign(poly.coefs[0]) + cstrain_core_Polynomial.getRepresentation(poly.coefs[0],0,varLabel)];
	var _g1 = 1;
	var _g = poly.coefs.length;
	while(_g1 < _g) {
		var i = _g1++;
		arr.push(cstrain_core_Polynomial.getSign(poly.coefs[i]) + cstrain_core_Polynomial.getRepresentation(poly.coefs[i],i,varLabel));
	}
	arr.reverse();
...
cstrain_vuex_game_GameGetters.Get_polynomialExpr = function(state) {
	if(state.showExpression) {
		if(state.polynomial != null) return cstrain_core_Polynomial.PrintOut(state.polynomial,"n"); else return "..";
	} else return "";
};
